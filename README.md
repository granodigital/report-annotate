...
When report files are found but there is nothing to report (no errors,
out-of-diff annotations, or skipped annotations) and a previous bot comment
exists, the action replaces it with an "All issues resolved" status — minimizing
the previous comment and posting a fresh all-clear (`minimize`), or updating the
